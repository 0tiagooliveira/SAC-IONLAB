...
class GestaoLogisticaAcaoExecutadaForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self._acao_confirmar_entrega_cliente():
            self.fields['data_efetiva_entrega'].label = 'Data da efetiva entrega no Cliente'
            self.fields['data_efetiva_entrega'].required = True
        ...
        # Blindagem Coleta na Ionlab: estes dados vêm da etapa anterior/histórico e não podem ser alterados pelo usuário.
        if self._acao_aguardando_coleta_ionlab():
            dados_travados = _extrair_dados_historico_logistica(self.sac)
            campos_travados = [
                'nome_transportadora',
                'telefone_transportadora',
                'numero_cotacao_frete',
                'valor_frete_cotado',
            ]
            for campo_travado in campos_travados:
                valor_travado = dados_travados.get(campo_travado)
                if valor_travado not in (None, '', []):
                    self.fields[campo_travado].initial = valor_travado

            valor_transportadora_travado = self.fields['nome_transportadora'].initial
            if valor_transportadora_travado:
                choices_existentes = list(self.fields['nome_transportadora'].choices)
                valores_existentes = {str(valor) for valor, _label in choices_existentes}
                if str(valor_transportadora_travado) not in valores_existentes:
                    choices_existentes.append((valor_transportadora_travado, valor_transportadora_travado))
                    self.fields['nome_transportadora'].choices = choices_existentes

            self.fields['nome_transportadora'].widget.attrs['disabled'] = 'disabled'
            self.fields['nome_transportadora'].widget.attrs['class'] = (self.fields['nome_transportadora'].widget.attrs.get('class', '') + ' campo-travado-logistica').strip()
            for campo_travado in ['telefone_transportadora', 'numero_cotacao_frete', 'valor_frete_cotado']:
                self.fields[campo_travado].widget.attrs['readonly'] = 'readonly'
                self.fields[campo_travado].widget.attrs['class'] = (self.fields[campo_travado].widget.attrs.get('class', '') + ' campo-travado-logistica').strip()

        if self._acao_aguardando_coleta_ionlab():
            self.fields['data_expedicao'].label = 'Data da efetiva coleta'
            self.fields['dias_entrega_cliente'].label = 'Tempo de entrega até o cliente'
            self.fields['data_prevista_chegada_cliente'].label = 'Data prevista para entrega'
            self._configurar_fluxo_coleta_ionlab_cliente({})
        elif self._eh_tipo_tecnico_equipamento_logistica():
    ...
    def _acao_confirmar_entrega_cliente(self):
    ...
    def _acao_aguardando_coleta_ionlab(self):
        texto = _normalizar_chave_fluxo(self.acao_atual_nome)
    ...
    def _resolver_fluxo_coleta_ionlab_para_cliente(self):
        status = _resolver_status_por_nome(
            'Em Trânsito (Com destino ao Cliente)',
            'Em Transito (Com destino ao Cliente)',
            'EM_TRANSITO_COM_DESTINO_AO_CLIENTE',
        )
        acao = _resolver_acao_por_nome(
            'Confirmar entrega realizada no cliente',
            'Confirmar Entrega Realizada no Cliente',
            codes=('CONFIRMAR_ENTREGA_REALIZADA_NO_CLIENTE', 'CONFIRMAR_ENTREGA_CLIENTE'),
        )
        setor = _resolver_setor_por_nome('Logística', 'Logistica', codes=('LOGISTICA',))
        return status, acao, setor

    def _configurar_fluxo_coleta_ionlab_cliente(self, cleaned_data):
        status, acao, setor = self._resolver_fluxo_coleta_ionlab_para_cliente()
    ...
    def clean(self):
        ...
        # Prioridade absoluta: Aguardando Coleta na Ionlab tem fluxo próprio e campos próprios.
        if self._acao_aguardando_coleta_ionlab():
            # Blindagem no backend: mesmo que alguém altere o HTML/DevTools, o sistema ignora o POST
            # e reaplica os valores oficiais vindos da etapa anterior/histórico.
            dados_travados = _extrair_dados_historico_logistica(self.sac)
            for campo in ['nome_transportadora', 'telefone_transportadora', 'numero_cotacao_frete', 'valor_frete_cotado']:
                valor = dados_travados.get(campo)
                if valor in (None, '', []):
                    valor = self.fields.get(campo).initial if self.fields.get(campo) else cleaned_data.get(campo)
                if valor not in (None, '', []):
                    cleaned_data[campo] = valor
                if valor in (None, '', []):
                    self.add_error(campo, 'Campo obrigatório.')
            for campo in ['data_expedicao', 'dias_entrega_cliente']:
                valor = cleaned_data.get(campo)
                if valor in (None, '', []):
                    self.add_error(campo, 'Campo obrigatório.')
            data_coleta = cleaned_data.get('data_expedicao')
            dias = cleaned_data.get('dias_entrega_cliente')
            if data_coleta and dias not in (None, ''):
                try:
                    cleaned_data['data_prevista_chegada_cliente'] = data_coleta + timedelta(days=int(dias))
                except Exception:
                    self.add_error('dias_entrega_cliente', 'Informe um número válido de dias.')
            status, acao_destino, setor_destino = self._configurar_fluxo_coleta_ionlab_cliente(cleaned_data)
            if not status or not acao_destino or not setor_destino:
                raise forms.ValidationError('Não foi possível resolver o fluxo de Coleta na Ionlab para entrega ao cliente.')
            return cleaned_data
