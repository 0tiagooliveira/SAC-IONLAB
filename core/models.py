...
class UsuarioSistema(models.Model):
    ...
    cpf = models.CharField(
        ...
        blank=True,
        null=True,
        help_text='Informe somente números. O sistema bloqueia CPF duplicado.'
    )
    ativo = models.BooleanField(default=True)
    setor = models.ForeignKey(Setor, on_delete=models.PROTECT, blank=False, null=True)
    ...
    class Meta:
        ...
        permissions = [
            ...
            ('cancelar_sac', 'Pode cancelar SAC'),
        ]
    ...
    def clean(self):
        super().clean()
        self.cpf = self.normalizar_cpf(self.cpf)
        if not self.nome_completo:
            nome = ' '.join([self.user.first_name or '', self.user.last_name or '']).strip()
            self.nome_completo = nome or self.user.get_username()
        if self.cpf and len(self.cpf) != 11:
            raise ValidationError({'cpf': 'CPF deve conter 11 números.'})
        if self.cpf and UsuarioSistema.objects.filter(cpf=self.cpf).exclude(pk=self.pk).exists():
            raise ValidationError({'cpf': 'Já existe usuário cadastrado com este CPF.'})
        if not self.setor_id:
            raise ValidationError({'setor': 'Setor é obrigatório para usuários do sistema.'})
...
class PermissaoTelaUsuario(models.Model):
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name='permissoes_telas_simples'
    )
    tela = models.CharField(max_length=80, choices=[(codigo, label) for codigo, label, _perm in TELAS_SISTEMA])
    nivel = models.CharField(max_length=20, choices=NIVEIS_ACESSO, default='sem_acesso')
    ...
    def __str__(self):
        return f'{self.user} - {self.get_tela_display()} - {self.get_nivel_display()}'


class PermissaoCadastroUsuario(models.Model):
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name='permissoes_cadastros_simples'
    )
    item = models.CharField(max_length=80, choices=CADASTRO_ITENS)
    nivel = models.CharField(max_length=20, choices=NIVEIS_ACESSO, default='sem_acesso')
    ...
    def __str__(self):
        return f'{self.user} - {self.get_item_display()} - {self.get_nivel_display()}'
...
class ItemNotaFiscal(models.Model):
    ...
    voltagem = models.CharField(max_length=80, blank=True, null=True)
    ...
    def save(self, *args, **kwargs):
        from core.services.cadastro_itens import normalizar_texto_cadastro, normalizar_voltagem
        ...
        if self.descricao_item:
            self.descricao_item = normalizar_texto_cadastro(self.descricao_item)
        if self.item:
            self.item = normalizar_texto_cadastro(self.item)
        if self.agrp:
            self.agrp = normalizar_texto_cadastro(self.agrp)
        if self.origem:
            self.origem = normalizar_texto_cadastro(self.origem)
        if self.modelo:
            self.modelo = normalizar_texto_cadastro(self.modelo)
        if self.voltagem:
            self.voltagem = normalizar_voltagem(self.voltagem)
        super().save(*args, **kwargs)
...
class PecaTabelaPreco(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.referencia:
            self.referencia = normalizar_referencia(self.referencia)
        if self.descricao:
            self.descricao = normalizar_texto_cadastro(self.descricao)
        if self.grupo:
            self.grupo = normalizar_texto_cadastro(self.grupo)
        if self.origem:
            self.origem = normalizar_texto_cadastro(self.origem)
        if self.modelo:
            self.modelo = normalizar_texto_cadastro(self.modelo)
...
class ImportacaoLegadoSAC(models.Model):
    STATUS_CHOICES = [
        ('DRY_RUN', 'Simulacao'),
        ('COMMIT', 'Importado'),
        ('ERRO', 'Erro'),
