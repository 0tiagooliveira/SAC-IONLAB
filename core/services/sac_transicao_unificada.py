...
from core.services.comunicacao_cliente import enviar_email_evento_sac
...
def _normalizar_texto_regra(valor: str) -> str:
    import unicodedata
...
def _aplicar_regra_ionlab_logistica(sac: SAC, acao: Optional[AcaoEmEspera], setor: Optional[Setor]):
    empresa_codigo = getattr(getattr(sac, 'empresa', None), 'codigo', '') or ''
    if empresa_codigo.upper() != 'IONLAB' or not acao:
        return acao, setor
    if _normalizar_texto_regra(acao.nome) != 'aguardando coleta no cliente':
        return acao, setor

    acao_nova = AcaoEmEspera.objects.filter(
        ativo=True,
        nome__iexact='Aguardando Cliente emitir Nota fiscal e fazer envio',
    ).select_related('setor_destino').first()
    if not acao_nova:
        return acao, setor
    return acao_nova, getattr(acao_nova, 'setor_destino', None) or setor
...
def _tipo_evento_email_sac(acao_executada_texto, status_anterior, status_final, setor_origem, setor_final):
    acao_texto = (acao_executada_texto or "").strip().lower()
    if "abertura" in acao_texto:
        return "abertura"
    if _status_eh_concluido(status_final):
        return "conclusao"
    if getattr(setor_origem, "id", None) != getattr(setor_final, "id", None):
        return "mudanca_setor"
    if getattr(status_anterior, "id", None) != getattr(status_final, "id", None):
        return "mudanca_status"
    return ""


def _mensagem_evento_email_sac(tipo_evento, status_anterior, status_final, setor_origem, setor_final):
    if tipo_evento == "abertura":
        return "Um novo SAC foi aberto e esta disponivel para acompanhamento."
    if tipo_evento == "conclusao":
        return "O SAC foi concluido."
    if tipo_evento == "mudanca_setor":
        origem = getattr(setor_origem, "nome", None) or "-"
        destino = getattr(setor_final, "nome", None) or "-"
        return f"O SAC mudou de setor: {origem} -> {destino}."
    if tipo_evento == "mudanca_status":
        anterior = getattr(status_anterior, "nome", None) or "-"
        novo = getattr(status_final, "nome", None) or "-"
        return f"O SAC mudou de status: {anterior} -> {novo}."
    return ""
...
def aplicar_transicao_sac(*, sac: SAC, usuario, acao_em_espera: Any = None, setor_destino: Any = None, status_novo: Any = None, observacao: Optional[str] = None, acao_executada_texto: Optional[str] = None, data_evento=None, salvar_sac: bool = True, arquivo_historico=None) -> ResultadoTransicaoSAC:
    ...
    acao, setor_resolvido = _aplicar_regra_ionlab_logistica(sac, acao, setor_resolvido)
    ...
    with transaction.atomic():
        ...
        tipo_evento_email = _tipo_evento_email_sac(
            acao_executada_texto,
            status_anterior,
            status_final,
            setor_origem,
            setor_final,
        )
        if tipo_evento_email:
            transaction.on_commit(
                lambda: enviar_email_evento_sac(
                    sac,
                    tipo_evento_email,
                    _mensagem_evento_email_sac(
                        tipo_evento_email,
                        status_anterior,
                        status_final,
                        setor_origem,
