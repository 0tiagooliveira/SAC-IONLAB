...
def _q_gestao() -> Q:
    return (
        _q_por_codigo_ou_nome('setor_atual', 'SAC') |
        _q_por_codigo_ou_nome('acao_em_espera__setor_destino', 'SAC') |
        _q_status('GESTAO_SAC') |
        _q_status('EM_ANALISE') |
        _q_acao('ANALISE_OCORRIDO_SAC') |
        _q_acao('AGUARDANDO_PEDIDO_ENTRADA_RETORNO')
    )


def _q_comercial() -> Q:
    return (
        _q_por_codigo_ou_nome('setor_atual', 'COMERCIAL') |
        _q_por_codigo_ou_nome('acao_em_espera__setor_destino', 'COMERCIAL') |
        _q_status('GESTAO_COMERCIAL') |
        _q_status('EMISSAO_PEDIDO_ENTRADA_RETORNO') |
        _q_status('EMISSAO_PEDIDO_DE_ENTRADA_RETORNO') |
        _q_acao('ANALISE_OCORRIDO')
    )


def _q_assessoria() -> Q:
    return _q_por_codigo_ou_nome('setor_atual', 'ASSESSORIA') | _q_por_codigo_ou_nome('acao_em_espera__setor_destino', 'ASSESSORIA')


def _q_logistica() -> Q:
    return (
        _q_por_codigo_ou_nome('setor_atual', 'LOGISTICA') |
        _q_por_codigo_ou_nome('acao_em_espera__setor_destino', 'LOGISTICA') |
        _q_acao('AGUARDANDO_EMISSAO_NF') |
        _q_acao('AGUARDANDO_COLETA_CLIENTE') |
        _q_acao('TRANSITO_IONLAB') |
        _q_acao('AGUARDANDO_DEVOLUCAO_CLIENTE') |
        _q_acao('TRANSITO_CLIENTE') |
        _q_status('AGUARDANDO_EMISSAO_NF') |
        _q_status('AGUARDANDO_COLETA_CLIENTE') |
        _q_status('TRANSITO_IONLAB') |
        _q_status('AGUARDANDO_DEVOLUCAO_CLIENTE') |
        _q_status('TRANSITO_CLIENTE')
    )


def _q_diretoria() -> Q:
    return (
        _q_por_codigo_ou_nome('setor_atual', 'DIRETORIA') |
        _q_por_codigo_ou_nome('acao_em_espera__setor_destino', 'DIRETORIA') |
        _q_acao('APROVACAO_DESCONTO') |
        _q_status('APROVACAO_DESCONTO')
    )


def _q_licitacao(setor: Optional[Setor] = None) -> Q:
    q = (
        _q_por_codigo_ou_nome('setor_atual', 'LICITACAO') |
        _q_por_codigo_ou_nome('acao_em_espera__setor_destino', 'LICITACAO') |
    ...
    if setor is not None:
        q |= Q(setor_atual_id=setor.id) | Q(acao_em_espera__setor_destino_id=setor.id)
    return q


def _marco_tempo_sac(sac: SAC):
    historico = None
    if hasattr(sac, 'historicos'):
        try:
...
def queryset_fila_por_setor(setor: Optional[Setor] = None, *, nome_setor: Optional[str] = None) -> QuerySet:
    ...
    if eh_setor_assessoria(codigo):
        ...
        if setor is not None:
            filtro |= Q(setor_atual_id=setor.id) | Q(acao_em_espera__setor_destino_id=setor.id)
    ...
    if eh_setor_assistencia_tecnica(codigo):
        if setor is not None:
            return qs.filter(Q(setor_atual_id=setor.id) | Q(acao_em_espera__setor_destino_id=setor.id)).order_by('data_abertura', 'id')
    if setor is not None:
        return qs.filter(Q(setor_atual_id=setor.id) | Q(acao_em_espera__setor_destino_id=setor.id)).order_by('data_abertura', 'id')
    return qs.none()
