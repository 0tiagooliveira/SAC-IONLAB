...
def popular_codigos_acaoemespera(apps, schema_editor):
    ...
    for acao in AcaoEmEspera.objects.all().order_by('id'):
        base = _normalizar_codigo(getattr(acao, 'codigo', None) or getattr(acao, 'nome', None) or f'ACAO_{acao.id}')
        if not base:
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('core', '0031_setor_acao_codigos'),
    ]

    operations = [
        migrations.RunPython(popular_codigos_acaoemespera, noop),
